...
const pollId = document.documentElement.dataset.pollId || "current";
const pollVotesCollection = collection(db, "households", householdId, "polls", pollId, "votes");
...
function normalizePlan(docSnap) {
  ...
  return {
    ...
    date: data.date || "",
    meal: data.meal || "",
    servings: Number(data.servings || 0) || undefined
  };
}
...
function savePlan(entry) {
  return setDoc(doc(plansCollection, entry.id), {
    ...
    date: entry.date || "",
    meal: entry.meal || "",
    servings: Number(entry.servings || 0) || null,
